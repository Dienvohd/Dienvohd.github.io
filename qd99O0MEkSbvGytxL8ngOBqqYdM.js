;if(CloudflareApps.matchPage(CloudflareApps.installs['HoK909GhtA4b'].URLPatterns)){(function(modules){var installedModules={};function __webpack_require__(moduleId){if(installedModules[moduleId]){return installedModules[moduleId].exports;}
...
var hash=void 0
var options=CloudflareApps.installs['HoK909GhtA4b'].options
var product=CloudflareApps.installs['HoK909GhtA4b'].product
...
if("HoK909GhtA4b"==='preview'&&node.nodeName==='IFRAME'&&node.src.indexOf('https://embedded.cloudflareapps.com')!==-1){return}
...
if("HoK909GhtA4b"==='preview'){messageIndex=previewMessageIndex}else{messageIndex=Math.floor(Math.random()*options.messages.length)}
...
if("HoK909GhtA4b"!=='preview'&&(!shouldShow||hasSeenHash())){hideWelcomeBar()
return}
updateElementStyle()
element.innerHTML=''
...
window.CloudflareApps.installs['HoK909GhtA4b'].scope={updateOptions:function updateOptions(nextOptions){if(nextOptions.messages.length!==options.messages.length){previewMessageIndex=nextOptions.messages.length-1}else{for(var i=0;i<nextOptions.messages.length;i++){var oldEntry=options.messages[i]
